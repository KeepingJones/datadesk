...
Current results (AI/Semi universe, momentum-core + bear overlay):
- Full period: CAGR 46.8%, Sharpe 1.61, MaxDD -26% vs SPY CAGR 15.2%, Sharpe 0.87
- Holdout (last 252d): Gate 1 Sharpe: **PASSES** (3.61 vs SPY 1.73) — but also state MaxDD **DOES NOT PASS** (-25.1% vs SPY -9%)
- **Caveat:** Survivorship-biased universe — absolute levels inflated until Tiingo/broader backfill

---

## Live Execution (Demo)

The OMS is shadow-first. Every signal generated by the platform is routed to the shadow audit log in `platform.db` without being sent to any broker unless explicitly armed.

**Deployment Gates (paper → live)**
1. **Gate 1 (backtesting):** Holdout Sharpe ≥ 1.0 AND MaxDD ≤ −20% AND beats SPY on both. *(Status: Sharpe PASSES, MaxDD DOES NOT PASS)*
2. **Gate 2 (paper validation):** 8+ weeks Alpaca paper or Trading212 demo — live results within 1 sigma of backtest expectation
3. **Gate 3 (live with cap):** Alpaca live US with capped capital; T212 ISA with manual-confirm before any automation
...
| **Daily rebalancer** | Fires at NYSE MOC window; queries `platform.db` for the highest-Sharpe 1y-holdout strategy (MaxDD ≥ −30%); routes per-ticker to the correct exchange |
...
| **Dashboard** | Dark ops-console UI — backtest leaderboard, equity curves, universe management, P&L summary, AI feed, Alpaca paper + T212 ISA status, Monte Carlo bootstrap simulations. ![Dashboard Screenshot](docs/assets/dashboard.png) [See UI Documentation](docs/ui_documentation.md) |
