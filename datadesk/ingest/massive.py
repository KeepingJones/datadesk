...
def backfill_massive(
    ...
    if api_key == "DEMO_KEY":
        logger.warning(
            "MASSIVE_API_KEY not set. Using DEMO_KEY, which will likely fail for non-AAPL tickers."
        )
        ...
        try:
            ...
            for attempt in range(max_retries):
                ...
                if status == "DELAYED" and "results" not in data:
                    ...
                    logger.warning(
                        f"Massive data DELAYED for {ticker}. Retry {attempt + 1}/{max_retries} in {wait_time}s..."
                    )
