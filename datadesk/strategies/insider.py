    ...
    def target_weights(prices: pd.DataFrame) -> pd.DataFrame:
        ...
        for ticker, group in insiders.groupby('ticker'):
            for i, row in group.iterrows():
                ...
                window = group[(group['observed_at'] <= row['observed_at']) & (group['observed_at'] > window_start)]
                if window['filer_name'].nunique() >= insider_min_buyers:
                    cluster_signals.append({'ticker': ticker, 'signal_date': row['observed_at'], 'type': 'insider'})
                    
        # --- Compute Congress Signals ---
        for i, row in congress.iterrows():
            cluster_signals.append({'ticker': row['ticker'], 'signal_date': row['observed_at'], 'type': 'congress'})
